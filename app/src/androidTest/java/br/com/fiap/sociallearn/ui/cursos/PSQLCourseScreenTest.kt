...
class PSQLCourseScreenTest {
    @get:Rule
    var activityRule: ActivityTestRule<PSQLCourseActivity>
            = ActivityTestRule(PSQLCourseActivity::class.java)
    ...
    fun validaCarregamentoInformacao() {
        ...
        Espresso
            .onView(
                ViewMatchers.withId(R.id.ivCursoPSQL)
            )
            .check(
                ViewAssertions.matches(ViewMatchers.isDisplayed())
            )
